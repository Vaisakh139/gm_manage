...
export default function MembersList() {
  ...
  const load = useCallback(() => {
    gymOwnerApi.getMembers(search, page, 10).then((r) => setData(r.data.data));
  }, [search, page]);
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try { await gymOwnerApi.deleteMember(deleteId); showToast('Member deleted', 'success'); load(); }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h3 className="text-lg font-semibold text-gray-900">Members {data ? `(${data.totalElements})` : ''}</h3>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-x-auto">
        ...
        {data?.content.length === 0 && <p className="text-center text-gray-400 py-10">No members found.</p>}
      </div>

      {/* Pagination */}
      {data && data.totalPages > 1 && (
        <div className="flex items-center justify-between text-sm">
          <p className="text-gray-500">Showing {page * 10 + 1}–{Math.min((page + 1) * 10, data.totalElements)} of {data.totalElements}</p>
          <div className="flex gap-2">
