...
export default function UserManagement() {
  ...
  const [filter, setFilter] = useState<'ALL' | 'GYM_OWNER' | 'MEMBER'>('ALL');
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h3 className="text-lg font-semibold text-gray-900">Users ({filtered.length})</h3>
        <div className="flex gap-2">
          {(['ALL', 'GYM_OWNER', 'MEMBER'] as const).map((r) => (
            <button key={r} onClick={() => setFilter(r)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${filter === r ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
            ...
            {filtered.map((u) => (
              <tr key={u.id} className="hover:bg-gray-50">
                ...
                <td className="px-5 py-3">
                  <Badge label={u.role === 'GYM_OWNER' ? 'Gym Owner' : 'Member'} variant={u.role === 'GYM_OWNER' ? 'blue' : 'gray'} />
                </td>
                ...
                <td className="px-5 py-3">
                  <button onClick={() => toggle(u)}
                    className={`text-xs font-medium px-3 py-1.5 rounded-lg transition-colors ${u.active ? 'bg-red-50 text-red-700 hover:bg-red-100' : 'bg-green-50 text-green-700 hover:bg-green-100'}`}>
